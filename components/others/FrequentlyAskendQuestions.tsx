...
// Define TypeScript interfaces for Contentful data structure
interface FAQ {
  sys: {
    id: string;
  };
  ...
  useEffect(() => {
    const fetchFaqs = async () => {
      try {
        ...
        const { data } = await client.query<FaqQueryResponse>({
          query: FAQ_QUERY
        });
        
        console.log("Contentful response:", data);
        
        if (data && data.faqCollection && data.faqCollection.items) {
        ...
        setError("Failed to load FAQ data. Check console for details.");
        setLoading(false);
      }
    };
